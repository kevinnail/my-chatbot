...
describe('EmailMemory model', () => {
  ...
  describe('getEmailsNeedingAnalysis', () => {
    it('should return emails with high similarity scores', async () => {
      const userId = 'test_user_analysis_needed';
      const minSimilarity = 0.8;

      // High similarity email
      await EmailMemory.storeEmail({
        userId,
        emailId: 'high_sim',
        subject: 'High Similarity Email',
        sender: 'test@example.com',
        body: 'This should be analyzed',
        emailDate: new Date(),
        similarityScore: 0.9,
      });

      // Low similarity email
      await EmailMemory.storeEmail({
        userId,
        emailId: 'low_sim',
        subject: 'Low Similarity Email',
        sender: 'test@example.com',
        body: 'This should not be analyzed',
        emailDate: new Date(),
        similarityScore: 0.3,
      });
      ...
      expect(emailsNeedingAnalysis).toHaveLength(1);
      expect(emailsNeedingAnalysis[0]).toMatchObject({
        email_id: 'high_sim',
        selection_reason: 'web_dev',
      });
    });

    it('should return appointment-related emails regardless of similarity', async () => {
      const userId = 'test_user_appointments';
      const minSimilarity = 0.8;

      await EmailMemory.storeEmail({
        userId,
        emailId: 'appointment_email',
        subject: 'Doctor Appointment Reminder',
        sender: 'clinic@example.com',
        body: 'Your appointment is tomorrow',
        emailDate: new Date(),
        similarityScore: 0.2, // Low similarity but appointment-related
      });
